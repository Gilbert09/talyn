...
- **Billing** — free plan = **3 active tasks** (`pending|queued|in_progress`) per owner across all their workspaces; **Unlimited** = $15/mo (or $150/yr) via **Polar** (merchant of record). The provider-agnostic entitlement seam is `services/billing/entitlements.ts` (gate in `createCloudTask` + the retry/start/PATCH re-activation paths; `TaskLimitError` → 402 `code:'task_limit_reached'` → desktop UpgradeModal); Polar specifics live only in `services/billing/{polar,webhook}.ts` (webhook: `/api/v1/webhooks/polar`, raw-body, idempotent + order-safe). Enforcement runs ONLY when the all-or-nothing `POLAR_*` env group is set (absent = limits off — the dev default and the prod kill switch). Comp an account with `UPDATE users SET plan_override='unlimited' WHERE email='…'` — webhooks never touch that column. See Session 68 in [`docs/SESSIONS.md`](./docs/SESSIONS.md).
