...
- **New outbound HTTP** (a new external API/integration) → time the call and `debugBus.recordHttp({ service, method, url, status, durationMs, ok, error? })` at the central request funnel (see `github.ts` `apiRequest`/`executeGraphql`, `posthogCode/client.ts` `request`). Add a one-liner to `SERVICE_INFO` in `DebugPanel.tsx`. If the API returns rate-limit headers, also `debugBus.recordRateLimit({ name, description, limit, remaining, used, resetAt, resource? })` off the response (see `github.ts` `recordRateLimit`, which parses `x-ratelimit-*` for the separate REST + GraphQL buckets) — it surfaces as a card in the panel's "API rate limits" row, refreshed by the 3s snapshot poll.
