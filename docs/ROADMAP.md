...
- [ ] **Complete prospective outcomes and provenance.** Join complete review history to earlier snapshots. Record authoritative request rounds and backend model versions. Treat exposure, opens, and submitted reviews as separate observations.
...
- [ ] **Add activity and code features.** Test revision-specific content and recent work through temporal ablations. Intervening reviews do not prove that a pending PR was seen or rejected.
- [ ] **Compare neural ranking and an offline teacher.** Start after the prospective data passes completeness checks. Keep human outcomes as evaluation labels. Score every eligible candidate while queues remain manageable.
