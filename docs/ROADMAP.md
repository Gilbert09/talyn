...
- [ ] **Decide whether Priority becomes the default.** It is opt-in, so it only helps once chosen. `pr_review_sort_mode_changed` is the evidence: if it is selected and not switched away from, promoting it is a one-line change.
