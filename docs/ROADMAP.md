...
- [x] **Review priority — a ranked Reviews tab, learned per person** (2026-09-21). A third sort mode behind the `reviewPriority` PostHog flag (**fallback OFF**, `availability: 'gated'`, audience: Tom only). A hard gate (`blocking_others` → `actionable` → `waiting_on_author` → `not_ready`) then points within it, computed client-side by `packages/shared/src/prPriority.ts` so the desktop and web forks cannot disagree about an ORDER. The relevance term is a per-viewer L2 pairwise-logistic model fitted from that person's own GitHub review history (`services/reviewPriority/{backfill,trainer,sweep}.ts`, migrations `0061`+`0062`), gated at 150 events and a 3pp lift over the shipped prior, clamped below the age ramp so affinity can never bury a PR indefinitely. Open follow-ups below.
