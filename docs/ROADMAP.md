...
- [x] **Review-ranking benchmark and first pooled models** (2026-09-21). The maintained lab compares conditional logit, personal coefficients, LambdaMART, and CatBoost. CatBoost won validation and tied request recency on the later replay. The stricter direct-request cohort cannot be compared with the old 53.4%. The earlier recency-ceiling and production-uplift claims are withdrawn. See [`REVIEW_RANKING.md`](./REVIEW_RANKING.md).
- [x] **Local review snapshots** (2026-09-21). Web and desktop record full queues, visible rows, and in-app opens. Collection uses the existing `reviewPriority` audience. An export button downloads the bounded local log. A checker rejects incomplete snapshots. No new snapshot events leave the device automatically.
- [x] **Prospective ranking pipeline and shared neural model** (2026-09-21). Added scoped review collection, strict labels, local content encoding, and a shared queue model. Personal adjustments need a separate validation gain. Four forward windows and private JSON artifacts support the next experiment. Production ranking remains unchanged.
- [x] **Larger ranking replay and personal fallback experiment** (2026-09-21). Collected 51,765 repository histories and compared twelve models. Selected models gain 61 hits over recency across 997 informative choices. The equal-reviewer gain remains uncertain, and the focus reviewer regresses. An exploratory fallback retains 20 extra hits and preserves that reviewer's baseline. See the [report](../scripts/review-ranking/results/2026-09-21-repository.md). No model was promoted.

- [x] **Production ranking trial infrastructure** (2026-09-22). Add a fixed shared candidate, central observations, submission reconciliation, and descriptive reports. Keep readiness gates and score limits. Roll out through a separate candidate flag. See [the protocol](REVIEW_RANKING.md).

Follow-ups now that review priority has shipped:
- [x] **Exact production score replay** (2026-09-21). Record server profile hashes, numeric scoring inputs, and repository scope. Replay the actual serving scorer and comparator. Exclude unknown scope and agent-operated snapshots from human outcome attribution. A live capture check found 77 team requests among 79 candidates, outside the direct-request benchmark's scope.
- [x] **Longer local capture** (2026-09-21). Archive snapshots in IndexedDB for up to 30 days within a per-workspace size limit. Keep the smaller fallback log. Exports report retention losses and archive failures. Current content for 79 observed candidates was captured and encoded locally for later observations.
- [x] **Review decision timing** (2026-09-21). Record creation and submission separately. Use earlier snapshots for ranking choices. Keep unfinished reviews as censoring evidence and session conversion as a separate measure. Twenty live identity and timestamp checks matched stored history.
- [x] **Observed team requests and batched outcomes** (2026-09-22). Retain matched team names in private local snapshots. Read scoped reviews across batches of 50 PRs, with pagination and identity checks. Keep the REST collector for independent comparisons. Request events alone failed to reproduce current team requests on seven of 79 audited PRs.
- [x] **Preserve active direct requests after prior reviews** (2026-09-22). Correct poll reconciliation and webhook refreshes. A live check found two direct requests that the previous rule would exclude. Comment-only reviews can leave a direct request active, which also limits the historical replay's request-round policy.
- [ ] **Collect prospective outcomes and complete provenance.** Use the collector on fresh human decisions. Record authoritative request rounds. Treat exposure, opens, and submitted reviews as separate observations.
- [ ] **Evaluate the complete production order.** Include readiness gates, score caps, and waiting-time policy. Compare PR creation recency and request recency separately. The component benchmark does not establish a defect in the age ramp.
- [ ] **Measure activity and content features.** The lab now supports observed content vectors and recent reviews. Test them on fresh decisions. Compare code-specialized encoders next. Intervening reviews do not prove exposure.
- [ ] **Measure shared neural ranking, then compare attention and a teacher.** The shared network and guarded personal adjustments are implemented. Fresh outcomes must establish their value. Keep human outcomes as evaluation labels and score complete queues.
- [ ] **Validate a future winner.** Freeze a future evaluation period and target a five-point Hit@3 gain over the strongest baseline. Require a positive paired confidence bound, then useful product outcomes before promotion.
- [ ] **Status pills on the Reviews row.** The `mine` variant has rendered `PRReviewPill` + `PRStatusPill` + the unresolved-threads chip all along; the `review` variant renders only a requester chip, so the tab hides the very state you triage on. Needs no flag and is worth doing on its own merits.
- [x] **Make Priority the default for all users.** New client builds reset the old sort preference once. Later choices remain saved.
- [ ] **Snooze / pin / dismiss.** Gmail's ablation found personalising the THRESHOLD beat personalising the model. A sort has no threshold, so it does not transfer today — but it would the moment this becomes a bucketed or filtered view.
- [ ] **Announce it.** `availability: 'gated'` withholds every `reviews`-scoped highlight from everybody until it flips to `'general'`, which is also what replays the withheld backlog.
...
- **Pending-failure follow-up (Session 118)**: `toPublicMergeQueue` publishes trunk's `pending_failure` as `testing`, because desktop builds from before Session 118 crash on a state they don't know. Drop `publicExternalState` once those builds have aged out; the amber "Queue: pending failure" pill then shows on the comment channel too, not only off the `trunk-pending-failure` label.
- **Stack batch follow-ups (Session 115)**: when the external queue ejects a batch, the fix run is dispatched at the SUBMITTED rung even though trunk's bisection may blame a lower one — the run is handed the stack in its prompt and has to place the fix itself. Routing it at the rung trunk names is the obvious next step. A covered rung's plan is also only refreshed by the 60s reconciler, so "queued with #N" can lag a batch that ended by up to a minute — waking the sibling groups directly was tried and reverted, because detached walks that schedule each other never let the process go idle (it hung CI for 100 minutes; see Session 115).
- **Merge stack follow-ups (Session 85, now only the SERIAL drain)**: batching bypasses all of this where the repo's queue takes stacks — it is what a branch-shaped stack, an ungated base, and a repo whose provider refused the batch still run. Stack enqueue inherits the workspace merge method, and `squash` is the hazard — a squash-landed parent leaves its commits only on the child's branch, so the retarget's `update_branch` conflicts and a paid cloud fix run does the rebase (it gets an explicit hint, but defaulting stack enqueue to `merge` would sidestep it entirely — open product call). No production exercise yet: the drain is covered end-to-end against pglite with a mocked GitHub, but nobody has pointed it at a real stack. The reconciler has no counter for entries parked in `awaiting_stack` beyond a day.

**Resolved:**
- ~~Stacked PRs have to be landed by hand~~ — Session 85: the merge queue parks each member in `awaiting_stack` behind the PR its base belongs to, retargets it onto the real base when that lands, and drains the chain bottom-up. `POST /:id/merge-queue/stack` queues the whole thing in one call. **Session 115**: where the branch the stack lands on is behind a merge queue that takes stacks (trunk.io + GitHub native stacked PRs), the whole stack is handed over as ONE submission instead — the provider tests and lands every rung in a single CI round, so an N-deep stack costs one test cycle rather than N.
