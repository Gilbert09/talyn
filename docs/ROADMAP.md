...
- [x] **Review-ranking experiment — does a better model exist?** (2026-09-21). An offline lab over 7 PostHog reviewers and ~350 GraphQL points, three-way time split, findings written up in [`REVIEW_RANKING.md`](./REVIEW_RANKING.md). Headline: the model above is worth **+23 points of top-3 over the cold-start prior once it fits**, but **plain newest-first is within 1.5 points of the best model found**, and a perfect recency oracle tops out at 50.4% — recency is saturated, so a single model fighting it head-on has almost nothing left to win. The lab itself is a throwaway spike in the gitignored `scripts/spikes/review-rank-lab/`; the doc is the durable artifact.

Follow-ups now that review priority has shipped:
- [ ] **Adopt pass-over count and scope affinity** ([`REVIEW_RANKING.md`](./REVIEW_RANKING.md)). The two strongest features the experiment found, both computable from data already fetched. Pass-over count — how many times the viewer reviewed something *else* while this PR waited — ranks second only to author affinity, and each pass-over is an explicit decision to skip.
- [ ] **Fix the age ramp's sign.** `agePoints` was built to favour OLDER PRs. Oldest-first scores 3.9% top-3, five times *worse* than random, because age does not mean "most owed" — it means "already declined, N times". This is a live defect in the shipped scorer, not a missed opportunity.
- [ ] **Drop `repoAffinity`; reconsider `pathFamiliarity` and `teamAffinity`.** Near-zero learned weight and they spend parameter budget the fit cannot afford. Note `queueDepth`/`inSession`-style features measure *nothing* by construction: a feature constant within a choice set cannot rank it.
- [ ] **Try the two-stage cascade** — recency selects the fresh cohort, affinity re-ranks within it. The most promising untried idea, and it follows directly from recency being saturated. If it does not clear newest-first by a meaningful margin, the finding is that a good recency sort plus a light affinity tiebreak is the right product.
- [ ] **Re-examine the absence correction and the 90-day half-life.** Both shipped on intuition the same week. The experiment measures the absence correction at **−0.0 points**, and every half-life from 7 days to never lands within 1.4 points of the others.
