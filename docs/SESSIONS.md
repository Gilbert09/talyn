...
**The dedup discarded most of the stream.** `streamer.ts` dedups SSE frames by `id:` through `streamIdGreaterThan`, written for a Redis stream id (`<ms>-<seq>`, both halves numeric) with a string compare as the fallback for anything else. Reading posthog's own `products/tasks/backend/` settles what it actually sends, and it is two id spaces, not one: a thin-tail run first replays its history out of the durable run log under synthetic **`log-<n>`** ids (`logic/stream/backlog.py`), then attaches the live Redis tail. Both took the string path. Inside the replay, `log-10` sorts below `log-9`, so entry 10 was dropped as already seen and so was everything up to 89. Worse is the handover: the cursor is then `log-<n>` and every following id is a Redis id, and `'1'` sorts below `'l'` — so not one live event was accepted AND the cursor never advanced off `log-<n>`. That loses the whole live tail rather than a slice of it, which is why the pane was blank rather than visibly lossy. The same arithmetic applies to the agent's own `<boot>-<seq>` ids, which is what `session_logs` entries carry (verified against a real 383-entry run through the tasks API). `streamIdIsNew` splits at the last hyphen and compares the counter numerically; an id it cannot order counts as NEW, and that fail-safe is what carries the backlog→live handover, the two spaces being incomparable by construction. Duplicates are the acceptable direction here — PostHog's own endpoint docs say a Redis-id reconnect "may re-deliver a few events already served as backlog frames". The suite stayed green because its fixtures used `1-0`, `2-0`, `3-0`: Redis-shaped ids that take the numeric path and never cross a digit boundary.

**Then the recovery path was gated on the wrong question.** A terminal run with no stored log is re-read from the durable S3 log, and both gates — the provider's and the cloud poller's 30-minute re-selection window — asked whether `transcript` was EMPTY. A live stream stopped early settles its buffer into the column on the way out, and the watch TTL lapsing (the viewer navigated away) is the ordinary way that happens. One flushed fragment of a thought made the transcript non-empty, so the task was permanently ineligible for the backfill that would have replaced it with the real log: it kept a fragment nobody can read, forever. Emptiness says "we stored nothing"; the decision needs "what we stored is the record". That is now `metadata.transcriptFinal` (`transcriptStore.ts`), written only by a read that went through `converter.end()` — a seed for a live tail is explicitly not final — and cleared by every path that points an existing row at a new or resumed run (redispatch rebuilds metadata anyway; quota failover, both retry routes and PostHog's `maybeRevive` clear it explicitly). The fleet marks it on its terminal tick, which keeps a finished fleet task out of the window rather than re-finalising it every tick for half an hour.

**The window now covers every terminal status, which needed three other things to be true.** It used to match `completed` alone, so a FAILED run — the one whose log somebody actually needs — got the single attempt at the instant the S3 flush was least likely to have landed. Widening it meant (a) measuring the window from `updatedAt`, because `completedAt` is null on every terminal status but one; (b) `patchTaskMetadata` no longer writing when a patch hands its input straight back, or a no-op write per tick would renew an `updatedAt`-bounded window forever (reference equality, not a deep compare — everything that means to write builds a new object); and (c) PostHog's reconcile returning early on an already-terminal task, since everything past that point ends in `finalize`, which re-emits the status and re-files the outcome analytics. That early return also made the common case cheaper: we hold the run id and already know the run is over, so a finished task backfills without asking the vendor anything. The resume re-check is now gated on `metadata.reviveEligible` rather than on the status, which is what stops ~180 pointless requests per finished task across the window.
...
than the build. Lift the mapping once pre-119 builds have aged out; until then
