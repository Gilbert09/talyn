...
- **Follow-up (same session): merge-queue infinite 403 loop on PostHog/posthog#67815.** Diagnosis: the bot merge is refused by PostHog's `master` ruleset (Talyn App not on the bypass list — expected, June 23 design), but the user-token fallback is now dead: since the June 16 App-only cutover the stored user token is a `ghu_` user-to-server token, which GitHub treats as the *same integration* → identical `Resource not accessible by integration` 403 (verified: App + installation permissions are sufficient, the PR was approved/human-mergeable). The queue then looped `waiting → refetch → clean → re-merge` forever. Fix: `MergeNotPermittedForAppError` thrown from `mergePullRequest` when every token flavour gets the integration-403; the merge queue treats it as terminal — `blocked` + persisted `mergeForbidden` flag (gates the clean-path merge), one-time `notifyBlocked` with an actionable reason (merge manually / add "Talyn App" to the ruleset bypass list), turn advances; dequeue/requeue resets. Real-world unblock for PostHog repos: ask an org admin to add the Talyn App to the `master` ruleset bypass list.
