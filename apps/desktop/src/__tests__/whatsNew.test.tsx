...
import { useWhatsNew, LAST_SEEN_KEY, CURSORS_KEY } from '../renderer/hooks/useWhatsNew';
...
describe('useWhatsNew — deciding whether to interrupt', () => {
  ...
  it('replays the backlog on the first launch after a feature is released', async () => {
    ...
    await waitFor(() => expect(modalOpen()).toBe(true));
    expect(screen.getByText('Run a prompt on a schedule')).toBeInTheDocument();
