 ...
 * covers the block + inline constructs Claude actually emits in a
 * conversation: fenced code, headings, bullet / numbered lists,
 * blockquotes, horizontal rules, and inline code / bold / italic /
 * links. Anything it doesn't recognise falls through as a plain
 * paragraph, so it can never render worse than raw text.
 */
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('```')) {
      ...
      parts.push(
        ...
        >
          {lang && (
            <div className="text-[10px] uppercase tracking-wide text-zinc-500 mb-1">{lang}</div>
          )}
          {codeLines.join('\n')}
        </pre>
      );
      ...
      const sizeClass =
        level <= 1
          ...
          : level === 2
            ...
            : 'text-sm font-medium';
      parts.push(
        <div key={`h-${parts.length}`} className={cn('mt-2 mb-1 text-zinc-100', sizeClass)}>
    ...
    if (/^(---|\*\*\*|___)\s*$/.test(line)) {
      flushParagraph();
      parts.push(<hr key={`hr-${parts.length}`} className="my-2 border-zinc-700/60" />);
      i++;
      continue;
    }
    ...
    if (/^>\s?/.test(line)) {
      ...
      parts.push(
        <blockquote
          key={`q-${parts.length}`}
          className="my-1 border-l-2 border-zinc-600 pl-3 text-zinc-300 [overflow-wrap:anywhere]"
        >
    ...
    if (m[1]) {
      ...
      nodes.push(
        <a
          ...
          className="text-blue-400 underline underline-offset-2 hover:text-blue-300 [overflow-wrap:anywhere]"
        >
          {m[2]}
        </a>
      );
    } else if (m[4] || m[5]) {
      nodes.push(<strong key={key++} className="font-semibold">{renderInline(m[4] ?? m[5])}</strong>);
    } else if (m[6] || m[7]) {
      nodes.push(<em key={key++}>{renderInline(m[6] ?? m[7])}</em>);
    }
    last = re.lastIndex;
  }
