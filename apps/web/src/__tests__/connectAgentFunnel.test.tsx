...
import { ConnectAgentModal } from '../components/modals/ConnectAgentModal';
...
type Harness = {
  ...
  createPostHogTask: ReturnType<typeof vi.fn>;
  runSkillTask: ReturnType<typeof vi.fn>;
  closeConnectAgent: ReturnType<typeof vi.fn>;
};
...
const h = (): Harness =>
  ((globalThis as unknown as { __talynConnect?: Harness }).__talynConnect ??= {
    ...
    runSkillTask: vi.fn(async () => true),
    closeConnectAgent: vi.fn(),
  });

vi.mock('../lib/analytics', () => ({ trackEvent: vi.fn() }));
vi.mock('../components/panels/SettingsPanel', () => ({ ProviderConnectCards: () => null }));
...
beforeEach(() => {
  ...
  s.createPostHogTask = vi.fn(async () => true);
  s.runSkillTask = vi.fn(async () => true);
  s.closeConnectAgent = vi.fn();
});
afterEach(() => {
  cleanup();
  vi.clearAllMocks();
...
describe('connect_agent_dispatched — the ask paying off', () => {
  ...
  it('stays silent when the dispatch itself declines to run', async () => {
    Object.assign(h(), {
      ...
      providerReady: true,
      createPostHogTask: vi.fn(async () => false),
    });
    render(<ConnectAgentModal />);
